...
function build_postbit($post, $post_type=0)
{
	...
	$likes = array();
	$query_likes = $db->query(" SELECT * FROM mybb_sg_sg_likes WHERE pid='$post_pid' ");
	$query_is_liked = $db->query(" SELECT * FROM `mybb_sg_sg_likes` WHERE pid='$post_pid' AND liked_by_uid=$user_uid; ");
	...
	if ($likes_number > 0) {
		$nombresLike = substr("$nombresLike", 0, -2); 
	}
	
	$likes_json = json_encode($likes);

	$query_personaje = $db->query("
		SELECT * FROM mybb_sg_sg_thread_personaje WHERE tid='$post_tid' AND uid='$user_fid'
	");
	...
	while ($c = $db->fetch_array($query_clan)) {
		$post['nombreClan'] = ucwords($c['nombreClan']);
	}
	
	if ($sumaStats > 560) { $clase = 'S+'; } 
	else if ($sumaStats > 480) { $clase = 'S'; } 
	else if ($sumaStats > 400) { $clase = 'A+';	} 
	else if ($sumaStats >= 320) { $clase = 'A';	} 
	else if ($sumaStats >= 240) { $clase = 'B';	} 
	else if ($sumaStats >= 160) { $clase = 'C';	} 
	else if ($sumaStats >= 80) { $clase = 'D'; }

	$post['clase'] = $clase;
	$post['villa_color'] = '--npcs-group-color';
	$post['villa_sombra1'] = '#000'; 
	$post['villa_sombra2'] = 'red';
	...
	if ($post['usergroup'] == 3 || $post['usergroup'] == 4) { $post['villa_color'] = '--staff-group-color'; $post['villa_sombra1'] = '#346971'; $post['villa_sombra2'] = '#346971'; } 
	if ($post['usergroup'] == 10) { $post['villa_color'] = '--suna-group-color'; $post['villa_sombra1'] = '#000'; $post['villa_sombra2'] = '#000'; } 
