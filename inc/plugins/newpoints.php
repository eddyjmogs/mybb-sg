...
function newpoints_addpoints($uid, $points, $forumrate = 1, $grouprate = 1, $isstring = false, $immediate = true)
{
	...
	$experiencia_limite_query = $db->query(" SELECT * FROM mybb_sg_sg_experiencia_limite WHERE uid='$uid' AND semana='$weekNumber'; ");
	...
	if ($doesLimitExistByWeek == false) {
		$db->query(" 
			INSERT INTO `mybb_sg_sg_experiencia_limite`(`uid`, `semana`, `experiencia_semanal`) VALUES ('$uid','$weekNumber','$experienciaSemanal')
		");
	}
	...
	// if ($experienciaSemanal + $pointsAwarded > 150) {
		...
		{
			$rinExperiencia = $rinExperiencia / 2;
		}
		...
		{
			$rinExperiencia = $rinExperiencia / 1.5;
		}

		$rinExperiencia = $rinExperiencia;
		
		$rins = 0;
		$query_ficha = $db->query(" SELECT * FROM mybb_sg_sg_fichas WHERE fid='$uid' "); 
		while ($f = $db->fetch_array($query_ficha)) { $rins = intval($f['rin']); }
		$nuevosRins = $rins + $rinExperiencia;
		$db->query(" UPDATE `mybb_sg_sg_fichas` SET `rin`='$nuevosRins' WHERE fid='$uid' ");
	...
	$db->query(" 
		UPDATE `mybb_sg_sg_experiencia_limite` SET `experiencia_semanal`='$nuevaExperienciaSemanal' WHERE uid='$uid' AND semana='$weekNumber';
	");
	...
	$rin_combinado = $rins_actual + $rinExperiencia;

	$log_expe = "$pointsAwarded de Experiencia ($exp_actual + $pointsAwarded = $exp_combinada) y $rinExperiencia Rins ($rins_actual + $rinExperiencia = $rin_combinado). [$threadid - $subject]";
	$db->query("
