...
// ── Total + paginación ────────────────────────────────────────
$total = 0;
$qc = $db->query("SELECT COUNT(*) AS c FROM mybb_sg_sg_audit_consola_mod $where_sql");
...
while ($l = $db->fetch_array($query_log)) {
    ...
    $username = htmlspecialchars($l['username'], ENT_QUOTES);
...
eval('$logs_li = $logs;');
eval("\$page_html = \"".$templates->get("staff_log_consola_mod")."\";");
