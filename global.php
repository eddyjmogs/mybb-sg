...
$banner_number = rand(1, 10);
$banner_easter = rand(1, 50);
$banner_horny_number = rand(1, 71);
$banner_hitsu_number = rand(1, 69);
$banner_nami_number = rand(1, 9);
$banner_horny_or_not = rand(1, 2);
...
$g_uid = $mybb->user['uid'];
$g_username = $mybb->user['username'];
$g_is_banner_horny = ($g_uid == '251' || $g_uid == '239');
$g_is_banner_hitsu = false;
$g_is_banner_nami = false;
$g_christmas = rand(1, 2);
...
if (is_staff($g_uid) || is_mod($g_uid) || is_user($g_uid)) {
	...
	$g_query_peticiones = $db->query("
		SELECT count(*) as numero_de_peticiones FROM `mybb_sg_sg_peticiones` WHERE resuelto=0
	");
	$g_query_fichas_en_cola = $db->query("
		SELECT count(*) as fichas_en_cola FROM `mybb_sg_sg_fichas` WHERE moderated = 'no_moderacion'
	");

	$query_total_creaciones_sin_moderar = $db->query("
		SELECT COUNT(*) as total_creaciones FROM mybb_sg_threads as t
	...
	// $query_total_tecnicas_nerfeo = $db->query("
	// 	SELECT count(*) as total_nerfeos FROM `mybb_sg_sg_tecnicas` WHERE balance_prioridad > 0
	// ");
	...
	// while ($q = $db->fetch_array($query_total_tecnicas_nerfeo)) {
	// 	$g_total_nerfeos = $q['total_nerfeos'];
	// }
...
if ($mybb->user['uid'] != 0) {
	...
	if ($mybb->user['uid'] == 482) {
		$g_queries = json_encode($mybb->input);

		if ($current_page == 'showthread.php') {
			$pages_tid = $mybb->input['tid'];
			$query_thread_subject =  $db->query(" SELECT subject FROM `mybb_sg_threads` WHERE tid='$pages_tid'; ");
			while ($q = $db->fetch_array($query_thread_subject)) {
				$subject = $q['subject'];
			}
			$g_queries .= " - $subject";
		}
		$db->query("INSERT INTO `mybb_sg_sg_pages`(`queries`, `url`, `username`, `uid`) VALUES ('$g_queries','$current_page','$g_username','$g_uid');");		
	}

	$query_hentai = $db->query(" SELECT * FROM mybb_sg_sg_hentai WHERE uid='$g_uid' ");
	while ($q = $db->fetch_array($query_hentai)) { $g_enable_hentai = $q['enable_hentai']; }

	$g_user_query = $db->query(" SELECT * FROM mybb_sg_users WHERE uid='$g_uid' ");
	$g_ficha_query = $db->query(" SELECT * FROM mybb_sg_sg_fichas WHERE fid='$g_uid' ");
	while ($q = $db->fetch_array($g_ficha_query)) { $g_tiene_ficha = true; $g_ficha = $q; }
	while ($q = $db->fetch_array($g_user_query)) { $g_user = $q; }

	$g_entrenamiento_en_curso = false;
	$g_entrenamiento_completo = false;
	$g_mision_en_curso = false;
	$g_mision_completa = false;	
	$g_recompensa_en_curso = false;
	$g_recompensa_completa = false;
	
	$g_usergroup = $g_user['usergroup'];
	$g_avatar = $g_user['avatar'];
	if (substr($g_user['avatar'], 0, 18) === './uploads/avatars/') {
		$g_avatar = substr($g_avatar, 1);
	}

	$g_villa_color = '--npcs-group-color';
	$g_villa_sombra1 = '#fff'; 
	$g_villa_sombra2 = '#fff';

	if ($g_usergroup == 9) { $g_villa_color = '--konoha-group-color'; $g_villa_sombra1 = '#3e5d1d'; $g_villa_sombra2 = '#2f4616'; } 
	if ($g_usergroup == 8) { $g_villa_color = '--kiri-group-color'; $g_villa_sombra1 = '#527ca7'; $g_villa_sombra2 = '#3e5d7d'; } 
	if ($g_usergroup == 12) { $g_villa_color = '--nsa-group-color'; $g_villa_sombra1 = '#69394f'; $g_villa_sombra2 = '#4f2b3b'; } 
	if ($g_usergroup == 13) { $g_villa_color = '--renegados-group-color'; $g_villa_sombra1 = '#9c0c0c'; $g_villa_sombra2 = '#750909'; } 
	if ($g_usergroup == 14) { $g_villa_color = '--iwa-group-color'; $g_villa_sombra1 = '#6e370f'; $g_villa_sombra2 = '#52290b'; } 
	if ($g_usergroup == 15) { $g_villa_color = '--kumo-group-color'; $g_villa_sombra1 = '#a48a26'; $g_villa_sombra2 = '#7b681c'; } 
	if ($g_usergroup == 11) { $g_villa_color = '--narradores-group-color'; $g_villa_sombra1 = '#fff'; $g_villa_sombra2 = '#fff'; } 
	if ($g_usergroup == 3) { $g_villa_color = '--staff-group-color'; $g_villa_sombra1 = '#fff'; $g_villa_sombra2 = '#fff'; } 
	if ($g_usergroup == 10) { $g_villa_color = '--suna-group-color'; $g_villa_sombra1 = '#fff'; $g_villa_sombra2 = '#fff'; } 

	$global_uid = $mybb->user['uid'];
	// Revisar entrenamientos y misiones en curso
	$g_query_user_entrenamientos = $db->query("
		SELECT * FROM mybb_sg_sg_entrenamientos_usuarios WHERE uid='$global_uid'
	");
	...
	while ($e = $db->fetch_array($g_query_user_entrenamientos)) {
        ...
        $codigo_usuario = get_obj_from_query($db->query("
            SELECT * FROM mybb_sg_sg_codigos_usuarios WHERE uid='$global_uid' AND expiracion > $time_now
        "));
        if ($codigo_usuario) {
	...
	$g_query_user_mision = $db->query("
		SELECT * FROM mybb_sg_sg_misiones_usuarios WHERE uid='$global_uid'
	");

	while ($m = $db->fetch_array($g_query_user_mision)) {
		$efecto = 1;
        $time_now = time();
        $codigo_usuario = get_obj_from_query($db->query("
            SELECT * FROM mybb_sg_sg_codigos_usuarios WHERE uid='$global_uid' AND expiracion > $time_now
        ...
        if (time() > (intval($m['tiempo_finaliza']) - $extra_time)) {
            $g_mision_completa = true;
        } else {
            $g_mision_en_curso = true;
        }
	}    
	...
	$g_query_recompensa_actual = $db->query("
		SELECT * FROM mybb_sg_sg_recompensas_usuarios WHERE uid='$global_uid'
	");
