$(function ($) {
	...
	// CODE CHAKRA
	$.sceditor.command.set('chakra', {
		_dropDown: function (editor, caller) {
			var $content;

			$content = $(
				'<div> <div style="text-align: center; padding: 0px 0 10px 0;"><b>Código de Chakra</b></div>' +
				'<div>' +
				'<label for="mod1">Chakra actual:</label> ' +
				'<input type="text" id="mod1" />' +
				'</div>' +
				'<div>' +
				'<label for="mod2">Chakra máxima:</label> ' +
				'<input type="text" id="mod2" />' +
				'</div>' +
				'<div>' +
				'<input type="button" class="button" value="' + editor._('Insert') + '" />' +
			...
			$content.find('.button').on('click', function (e) {
				...
				if (mod1 !== undefined && mod1 !== "" && mod2 !== undefined && mod2 !== "") {
					editor.insert(`[chakra=${mod1},${mod2}]`);
				}
			...
			editor.createDropDown(caller, 'insertchakra', $content.get(0));
		},
		exec: function (caller) {
			$.sceditor.command.get('chakra')._dropDown(this, caller);
		},
		txtExec: function (caller) {
			$.sceditor.command.get('chakra')._dropDown(this, caller);
		},
